...
class PyramidVisionTransformerV2(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.head = nn.Sequential(
            nn.Linear(embed_dims[3], embed_dims[3]),
            nn.ReLU(),
            CosineMarginProduct(embed_dims[3], num_classes, s=1, m=0) if num_classes > 0 else nn.Identity()
        )
        #self.head = CosineMarginProduct(embed_dims[3], num_classes, s=1, m=0) if num_classes > 0 else nn.Identity()
