...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        patch_size1 = to_2tuple(patch_size//2)
        ...
        #self.proj2 = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size1, stride=patch_size1)
    def forward(self, x):
        B, C, H, W = x.shape
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks:
            ...
            blk_output.append(x)
        x = self.norm(x)
        return x[:, 0], blk_output, x[:, 1:]

    def forward(self, x):
        feature, blk_output, patch_output = self.forward_features(x)
        x = self.head(feature)
        return x, feature, patch_output
...
@register_model
def deit_tiny_patch16_224(img_size=224, embed_dim=192, pretrained=False, **kwargs):
    model = VisionTransformer(
        img_size=img_size, patch_size=16, embed_dim=embed_dim, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
...
@register_model
def deit_tiny_patch16_224_dropout(img_size=224, embed_dim=192, pretrained=False, **kwargs):
    model = VisionTransformer(
        img_size=img_size, patch_size=16, embed_dim=embed_dim, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True,
        drop_rate=0.2, attn_drop_rate=0.2,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
...
@register_model
def deit_small_patch16_224(img_size=224, embed_dim=384, pretrained=False, **kwargs):
    model = VisionTransformer(
        img_size=img_size, patch_size=16, embed_dim=embed_dim, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
...
@register_model
def deit_base_patch16_224(img_size=224, embed_dim=384, pretrained=False, **kwargs):
    model = VisionTransformer(
        img_size=img_size, patch_size=16, embed_dim=embed_dim, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
